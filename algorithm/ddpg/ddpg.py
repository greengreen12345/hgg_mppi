...
class DDPG(Agent):
    ...
    def step(self, states: torch.Tensor, explore=False, goal_based=False):
        ...
        if self.args['buffer'].steps_counter < self.args['warmup']:
            print("self.args['buffer'].steps_counter", self.args['buffer'].steps_counter)
            print("self.args['warmup']", self.args['warmup'])
            action = np.random.uniform(-0.001, 0.001, size=self.args['acts_dims'])

            if isinstance(action, np.ndarray):
                action = torch.tensor(action, dtype=torch.float32, device=self.device)

            print("warmup action1")
            return action
        # if self.args['goal_based']: obs = goal_based_process(states)
        if goal_based:
        ...
        # states = self._state_preprocessor(processed_states).to(device)
    ...
    def step1(self, states: torch.Tensor) -> torch.Tensor:
        """Process the environment's states to make a decision (actions) using the main policy
    ...
    def preprocess_obs1(self, obs):
        return {
            "observation": torch.tensor(obs["observation"], dtype=torch.float32),
            "achieved_goal": torch.tensor(obs["achieved_goal"], dtype=torch.float32),
            "desired_goal": torch.tensor(obs["desired_goal"], dtype=torch.float32)
        }
    ...
    def normalizer_update1(self, batch):
        ...
        if self._state_preprocessor:
            ...
            combined_obs = torch.cat([obs, obs_next], dim=0)
            # combined_obs = torch.cat([batch["states"], batch["next_states"]], dim=0)
            # combined_obs = torch.cat([batch["obs"], batch["obs_next"]], dim=0)
    ...
    def train(self, batch):
        ...
        with torch.no_grad():
            ...
            #print("sampled_next_states.shape:", next_states.shape)
