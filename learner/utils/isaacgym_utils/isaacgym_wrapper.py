...
class IsaacGymWrapper:
    ...
    def reset(self):
        ...
        # self.goal = self._sample_goal().clone()  # 根据你的需求实现采样目标的方法
        # self.goal = self.goal*10
        ...
        obs = self._get_obs()  # 获取观测的方法也需要根据具体情况来定义
        return obs

    def reset1(self):
        # 尝试重置模拟器。因为我们会随机初始条件，有可能会遇到数值问题（例如由于穿透或者万向锁）
        # 或者可能无法达到一个初始条件（例如一个对象在手中）。
        # 在这种情况下，我们就不断地随机化，直到最终达到一个有效的初始配置。

        # did_reset_sim = False
        # while not did_reset_sim:
        #     did_reset_sim = self._reset_sim()  # 你需要实现这个方法来尝试重置仿真并返回是否成功
    ...
    def _reset_sim(self):
        ...
        Returns True if the reset is successful, otherwise False.
        """
        try:
            if self._sim is None:
                print("Warning: Simulation instance is None, reinitializing...")
                self.start_sim()

            # Check if initial states exist
            if not hasattr(self, "initial_root_state") or not hasattr(self, "initial_dof_state"):
                print("Error: Initial states are not available, cannot reset.")
                return False

            print("Resetting actor root states...")
            self._root_state[:] = self.initial_root_state.clone().to(self.device)
            self._gym.set_actor_root_state_tensor(self._sim, gymtorch.unwrap_tensor(self._root_state))

            print("Resetting DOF states...")
            self._dof_state[:] = self.initial_dof_state.clone().to(self.device)
            self._gym.set_dof_state_tensor(self._sim, gymtorch.unwrap_tensor(self._dof_state))

            # Refresh tensors after resetting
            print("Refreshing tensors...")
            self._gym.refresh_actor_root_state_tensor(self._sim)
            self._gym.refresh_dof_state_tensor(self._sim)
            self._gym.refresh_rigid_body_state_tensor(self._sim)
            self._gym.refresh_net_contact_force_tensor(self._sim)

            # Advance simulation one step to ensure everything is stable
    ...
    def validate_simulation_state(self):
        """
        验证仿真状态是否合乎预期，这是一个虚构的示例函数，需要你根据实际情况来实现。
        """
        # 例如，检查某个关键对象是否在预期位置
    ...
    def _sample_goal(self):
        # 假设初始位置是已知的，这里使用一个随机向量代替
        initial_pos = torch.tensor([0.5, 0.5, 0.2], device=self.device)

        if self.has_object:
            goal = initial_pos + torch.rand(3, device=self.device) * (self.target_range * 2) - self.target_range
            goal[2] = self.height_offset  # 设置高度偏移
            if self.target_in_the_air and torch.rand(1).item() < 0.5:
                # 如果目标需要在空中，并且随机决定，增加一定的高度
                goal[2] += torch.rand(1, device=self.device).item() * 0.45
        else:
            # 如果没有物体，简单地生成地面目标位置
            goal = initial_pos + torch.rand(3, device=self.device) * (self.target_range * 2) - self.target_range
    ...
    # def _get_obs(self):
    #     # 获取机器人的位置和速度
    #     robot_pos = self.get_actor_position_by_name("robot_name")  # 假设机器人名为"robot_name"
    #     robot_vel = self.get_actor_velocity_by_name("robot_name")
    #
    #     if self.has_object:
    #         # 如果环境中有物体，获取物体的位置、方向、速度等信息
    #         object_pos = self.get_actor_position_by_name("object_name")  # 假设物体名为"object_name"
    #         object_vel = self.get_actor_velocity_by_name("object_name")
    #
    #         # 计算物体相对于机器人的位置
    #         object_rel_pos = object_pos - robot_pos
    #         object_rel_vel = object_vel - robot_vel
    #     else:
    #         object_pos = object_vel = object_rel_pos = object_rel_vel = torch.zeros(3, device=self.device)
    #
    #     # 组合所有观测信息
    #     obs = torch.cat([robot_pos, robot_vel, object_pos, object_rel_pos, object_vel, object_rel_vel])
    #
    #     # 假设有一个目标位置，可以在此定义
    #     achieved_goal = object_pos if self.has_object else robot_pos
    #
    #     # 返回一个字典，包含观测、实现的目标和期望的目标
    #     return {
    #         'observation': obs,
    ...
    def _get_obs1(self):
        """
        获取当前环境的观测信息，包括机械臂的位置、速度、物体状态（如果有），以及目标状态。
        """

        # 获取机械臂末端位置（gripper 位置）
        grip_pos = self.get_actor_position_by_name("panda")  # 机械臂末端位置
        grip_velp = self.get_actor_velocity_by_name("panda")  # 机械臂末端线速度

        # 获取机器人当前关节状态
        robot_qpos = self._dof_state[:, :self.num_dofs // self.num_envs]  # 机器人关节角度
        robot_qvel = self.robot_vel  # 机器人关节速度

        # 计算时间步长
        dt = self._gym.get_sim_params(self._sim).dt

        # 检查是否有物体
        if self.has_object:
            object_pos = self.get_actor_position_by_name("object0")  # 物体位置
            object_rot = self.get_actor_orientation_by_name("object0")  # 物体旋转（四元数）
            object_velp = self.get_actor_velocity_by_name("object0")  # 物体线速度
            object_velr = torch.zeros_like(object_velp)  # Isaac Gym 没有直接提供角速度

            # 计算物体相对机械臂的位置
            object_rel_pos = object_pos - grip_pos
            object_velp -= grip_velp  # 物体速度相对机械臂速度

        else:
            object_pos = torch.zeros(3, device=self.device)
            object_rot = torch.zeros(4, device=self.device)  # 四元数格式
            object_velp = torch.zeros(3, device=self.device)
            object_velr = torch.zeros(3, device=self.device)
            object_rel_pos = torch.zeros(3, device=self.device)

        # 机械爪状态
        gripper_state = robot_qpos[:, -2:]  # 获取机械爪的状态
        gripper_vel = robot_qvel[:, -2:] * dt  # 机械爪速度

        # 计算 `achieved_goal`
        if not self.has_object:
            achieved_goal = grip_pos.clone()
        else:
            achieved_goal = object_pos.clone()

        # 组合观测向量
        obs = torch.cat([
            grip_pos.flatten(),  # 机械臂末端位置
            object_pos.flatten(),  # 物体位置
            object_rel_pos.flatten(),  # 物体相对机械臂位置
            gripper_state.flatten(),  # 机械爪状态
            object_rot.flatten(),  # 物体旋转
            object_velp.flatten(),  # 物体速度
            object_velr.flatten(),  # 物体角速度
            grip_velp.flatten(),  # 机械臂末端速度
            gripper_vel.flatten(),  # 机械爪速度
        ])
        ...
        return {
            ...
            'desired_goal': self.goal.cpu().numpy().copy(),
        }
    ...
    def _get_obs2(self):
        ...
        获取当前环境的观测信息，包括机械臂的位置、速度、物体状态（如果有），以及目标状态。
        """
        # actor_handle = self._get_actor_index_by_name("panda")  # 根据名字拿 actor 索引
        # body_names = self._gym.get_actor_rigid_body_names(self.envs[0], actor_handle)
        #
        # for idx, name in enumerate(body_names):
        #     print(f"Index {idx}: {name}")
        end_effector_index = 6  # 假设 panda_hand 是第 7 个

        # print(self._rigid_body_state[:, end_effector_index, :3])  # 位置
        # print(self._rigid_body_state[:, end_effector_index, 3:7])  # 四元数

        self.robot_pos = self._rigid_body_state[:, end_effector_index, :3]
        #print("-----------self._rigid_body_state---------", self._rigid_body_state)
        # 计算 `achieved_goal`
        achieved_goal = self.robot_pos.clone()

        obs = torch.stack([self.robot_pos[:, 0],
                              self.robot_vel[:, 0],
                              self.robot_pos[:, 1],
                              self.robot_vel[:, 1]], dim=1)
    ...
    def _get_obs(self):
        """
        获取当前环境的观测信息：
        obs 包括末端执行器的位置、旋转（四元数）、线速度、角速度；
        achieved_goal 是末端执行器的位置。
        """
        ...
        print("self.ee_state.shape", self.ee_state.shape)
        # if torch.isnan(self.ee_state).any():
        #     # self.ee_state = torch.zeros(13, device=self.device)
        #     self.ee_state = torch.zeros_like(self.ee_state)
            # print("⚠️ 末端执行器状态包含 NaN，尝试补一步仿真...")
            # self._gym.simulate(self._sim)
            # self._gym.fetch_results(self._sim, True)
            # self._gym.refresh_rigid_body_state_tensor(self._sim)
            # self.ee_state = (self.get_actor_link_by_name("panda", "panda_leftfinger") +
            #                  self.get_actor_link_by_name("panda", "panda_rightfinger")) / 2

        # position = self.ee_state[:, 0:3]
        # orientation = self.ee_state[:, 3:7]
        # linear_velocity = self.ee_state[:, 7:10]
        # angular_velocity = self.ee_state[:, 10:13]
        ...
        print("ee_state", self.ee_state)

        # end_effector_index = 6  # panda_hand 在 rigid body 中的 index（请根据实际确认）
        #
        # # 从 rigid body state 中提取末端执行器的状态
        # end_effector_state = self._rigid_body_state[:, end_effector_index]  # shape: [num_envs, 13]
        # position = end_effector_state[:, 0:3]
        # orientation = end_effector_state[:, 3:7]
        # linear_velocity = end_effector_state[:, 7:10]
        # angular_velocity = end_effector_state[:, 10:13]

        # 构造 observation
        #obs = torch.cat([position, orientation, linear_velocity, angular_velocity], dim=1)
        obs = torch.cat([position, orientation, linear_velocity, angular_velocity], dim=1)  # shape: [num_envs, 13]



        # 设置 achieved_goal 为末端位置
        ...
        return {
            'observation': obs.cpu().numpy().copy(),
            'achieved_goal': achieved_goal.cpu().numpy().copy(),
            'desired_goal': self.goal.clone().cpu().numpy().copy()
        }
